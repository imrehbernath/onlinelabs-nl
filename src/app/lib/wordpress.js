...
/**
 * Get All Trainings
 * Returns all published trainings
 * 
 * @returns {Array} Array of training objects
 */
export async function getAllTrainings() {
  console.log('🎓 Fetching all trainings from WordPress...');
  ...
  try {
    ...
    console.log('✅ Trainings fetched:', data?.trainings?.nodes?.length || 0);
    return data?.trainings?.nodes || [];
  } catch (error) {
    console.error('❌ Failed to fetch trainings:', error.message);
    return [];
  }
}
 ...
 * For training detail pages - FULL DATA including all ACF fields
 * 
 * @param {string} slug - Training slug
 * @returns {Object} Training object with all details
 */
export async function getTrainingBySlug(slug) {
  console.log(`🎓 Fetching training: ${slug}`);
  ...
  try {
    ...
    console.log('✅ Training fetched:', data.training.title);
    console.log('📦 SEO data:', data.training.seo ? 'Found' : 'Not found');
...
/**
 * Get All Training Slugs
 * For static generation of training detail pages
 * 
 * @returns {Array} Array of slugs
 */
export async function getAllTrainingSlugs() {
  console.log('🎓 Fetching all training slugs...');
  ...
  try {
    ...
    return data?.trainings?.nodes?.map(node => node.slug) || [];
  } catch (error) {
    console.error('❌ Failed to fetch training slugs:', error.message);
    return [];
  }
}
